...
async function submitTest() {
    ...
    if (testSubmitted) return; // Nếu đã nộp bài, không làm gì cả
...
//let overallband_2 = 0;
    ...
    */
    ...
    let overallBandDiv = document.getElementById(`overall-band-${quizData.questions[i].id}`);
    ...
    <h3 style="color:red">B. Tổng kết điểm của bạn và nhận xét (Overall Band and Comment)</h3>
        <h4 style = "font-weight:bold">Task Achievement: ${task_achievement_part_1}</h4>
        <p>${task_achievement_comment}</p>
        <h4 style = "font-weight:bold">Coherence and Cohesion: ${coherence_and_cohesion_part_1}</h4>
        <p>${coherence_and_cohesion_comment}</p>
        <h4 style = "font-weight:bold">Lexical Resource: ${lexical_resource_part_1}</h4>
        <p>${lexical_resource_comment}</p>
        <h4 style = "font-weight:bold">Grammatical Range and Accuracy ${grammatical_range_and_accuracy_part_1}</h4>
        <p>${grammatical_range_and_accuracy_comment}</p>
 
        <h4 style = "font-weight:bold">Overall Band: ${overallband}</h4>
        <p>- User Level: ${userLevel}</p>
        <p>- Description:  ${band_description}</p>
...
*/
    ...
    detailScoreDiv.innerHTML += `<div id="ads-placement"> Advertising</div>`;
