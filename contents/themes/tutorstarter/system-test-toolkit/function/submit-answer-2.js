function submitButton() {
    ...
    for (var z = 1; z <= quizData.number_questions; z++) {
        ...
        // Check if the question is a completion type
        if (elements[0].type === 'text') {
           ...
           correctAnswerDiv.innerHTML += `Question ${z} (Correct Answer - Input) : ${correctAnswer.join(', ')}`;
            ...
            if (correctAnswer.includes(userAnswer.toLowerCase())) {
                ...
                // Display the correct answer after the user submits
    ...
    string_final_out_of_10 = "Điểm của bạn là " + (percent / 10).toFixed(2) + "/ 10" + "<br>" + "Số câu đúng là: " + number_of_correct_answer + "/ " + quizData.number_questions + "<br>" + "Thời gian làm bài: " + formattedTimeUsed;
    time_do_full_test = formattedTimeUsed;

    if (percent >= quizData.pass_percent) {
        string_final += "You pass the test!";
        document.getElementById("final-result").classList.add('pass');
    } else {
        string_final += "You don't pass the test!";
        document.getElementById("final-result").classList.add('fail');
    }
